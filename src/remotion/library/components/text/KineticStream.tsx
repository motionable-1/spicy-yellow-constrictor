...
const useStreamTiming = (
  ...
  const progress = interpolate(timeInSlot, [0, transitionDuration], [0, 1], {
    extrapolateRight: "clamp",
    easing: Easing.bezier(0.19, 1, 0.22, 1), // Expo-like ease
  });

  const isTransitioning =
    timeInSlot < transitionDuration && safeCurrentIndex > 0;
        ...
        <div
          style={{
            ...
            opacity: isTransitioning
              ? interpolate(easeProgress, [0, 0.3], [0, 1])
              : 1,
          }}
        >
          {currentWord}
        ...
        <div
          style={{
            ...
            transform: isTransitioning
              ...
              : `scale(${currentScale})`,
            opacity: isTransitioning
              ? interpolate(progress, [0, 0.8], [0, 1])
              : 1,
...
export const StompStream: React.FC<KineticStreamProps> = ({
  ...
  const scale = isTransitioning
    ? interpolate(stompProgress, [0, 0.3], [3, 1], {
        extrapolateRight: "clamp",
  ...
  const opacity = isTransitioning
    ? interpolate(stompProgress, [0, 0.1], [0, 1], {
        extrapolateRight: "clamp",
      })
    : 1;
  ...
  const shake =
    isTransitioning && stompProgress > 0.3 && stompProgress < 0.6
...
export const OutlineStream: React.FC<KineticStreamProps> = ({
  ...
  const fillProgress = isTransitioning
    ? interpolate(progress, [0.2, 0.6], [0, 1], {
...
export const ChromaticStream: React.FC<KineticStreamProps> = ({
  ...
  const opacity = isTransitioning
    ? interpolate(progress, [0, 0.5, 1], [1, 0.5, 1])
    : 1;
          ...
          <div
            style={{
              ...
              transform: isTransitioning
                ...
                : `translate3d(0, 0, 0)`,
              opacity: isTransitioning
                ? interpolate(progress, [0.4, 1], [0, 1])
                : 1,
              backfaceVisibility: "hidden",
              whiteSpace: "nowrap",
            }}
...
> = ({
  ...
  const getTransform = (type: "enter" | "exit", p: number) => {
    ...
    const currentSkew = withSkew
      ? interpolate(p, [0, 0.2, 0.8, 1], [0, skewMax, skewMax, 0], {
          extrapolateLeft: "clamp",
          extrapolateRight: "clamp",
        })
      : 0;
