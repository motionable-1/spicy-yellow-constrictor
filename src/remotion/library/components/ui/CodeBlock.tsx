...
const highlightCode = (code: string, theme: "dark" | "light") => {
  ...
  const tokens = code.split(
    /(\/\/.*|\/\*[\s\S]*?\*\/|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`[^`]*`|\b(?:import|export|from|const|let|var|function|return|if|else|for|while|switch|case|break|continue|try|catch|async|await|class|interface|type|extends|implements|new|this|super|void|null|undefined|true|false)\b|\b\d+\b|[{}()[\].,:;]|\s+)/g,
  );
  ...
  return tokens.map((token, i) => {
    ...
    if (token.startsWith("//") || token.startsWith("/*")) {
      ...
      token.match(
        /^(?:import|export|from|const|let|var|function|return|if|else|for|while|switch|case|break|continue|try|catch|async|await|class|interface|type|extends|implements|new|this|super|void|null|undefined|true|false)$/,
      )
    ) {
